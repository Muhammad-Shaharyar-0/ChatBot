...
def readData(fileName):
    path = pandas.read_excel(r'H:\Python\untitled1\sorted.xlsx', fileName)
...
def GetResponce():
    if len(Information) == 0:
        response = "You have not provided me with enough information to suggest you Please First Mention What Would You like To Buy"
    if flags[0]==0:
        response = "Please tell me would you like to buy a house or a flat"
    elif flags[1] == 0:
        response = "Please mention the city in which you would like to buy in"
    elif flags[2] == 0:
        if flags[3] == 0:
            response = "You can mention the Area(Marla or Kannal) and the price in which you would like to buy or i could start giving you suggections"
        else:
            response = "You can mention the Area(Marla or Kannal) of which you would like to buy or i could start giving you suggections"
    elif flags[3] == 0:
    ...
    if flags[0]==1 and flags[1]==1 and flags[2]==1 and flags[3]==1:
        response="Thank you for providing with all the information needed you can now begin asking me for options and i would try my best to statisfy you"
    return response
def chat():
    ...
    def keyup():
        ...
        if results[0][result_index] > 0.8:
                    ...
                    if "Suggest" == Tags[result_index]:
                        ...
                        if flags[0]==1 and flags[1]==1:
                            ...
                            if len(list)>index1:
                                ...
                                index1=0
                                flags=[0,0,0,0]
                                string="I couldn't find any property that met your requirment. You can try providing differnt information again "
                            response=string
                        else:
                            reponse= GetResponce()
                    else:
                        if "House" == Tags[result_index]:
                            flags[0] = 1
                            Information.insert(0, "House")
                            response = GetResponce()
                        elif "Flat" == Tags[result_index]:
                            flags[0] = 1
                            Information.insert(0, "Flat")
                            response=GetResponce()
                        else:
            ...
            if count ==0:
                T = Label(chatwindow, text="Are you a retard. Type something i can understand", width=40,bg="grey",wraplength=270)
            else:
